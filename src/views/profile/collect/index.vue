<template>
  ...
  <div class="collect" >
    <div class="top">
      <el-tag size="large" plain v-if="loading">正在拼命为您查询已收藏的文章...</el-tag>
      <el-tag size="large" plain v-else>您已经收藏了<b>{{total}}</b>篇文章</el-tag>
      <div class="switch">
        <el-switch
            v-model="switchShow"
            class="mb-2"
            width="100"
            :disabled="empty || !total || loading"
            @change="switchChange"
            active-text="仅自己可见"
...
//获取用户收藏文章列表
const getCollect = async (val?:string)=>{
