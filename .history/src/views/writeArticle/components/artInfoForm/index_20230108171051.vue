  ...
  <div class="artInfoForm">
    ...
    >
      ...
      <el-form-item label="内容详情" class="item"  prop="artIntroduce">
        <el-input  type="textarea" clearable v-model.trim="ruleForm.artIntroduce"  placeholder="请填写内容详情(简单描述文章主体内容)" type="text" autocomplete="off" />
      </el-form-item>
...
const validateArtTitle = (rule: any, value: any, callback: any) => {
  if (value === '') {
    ...
    if (value.length < 8 || value.length > 60) {
      callback(new Error('标题的字数应该在8-60之间'))
    }else {
    callback()
  }
}
...
const validateIntroduce = (rule: any, value: any, callback: any) => {
  if (value === '') {
    ...
    if (value.length < 8 || value.length > 120) {
      callback(new Error('描述的字数应该在8-200之间'))
    }else {
    callback()
  }
}
