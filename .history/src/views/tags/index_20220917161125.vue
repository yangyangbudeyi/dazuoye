<template>
  <div class="tags" >
    <div class="top">
      <span >为您展示<b>{{obj.key}}</b>标签的相关文章</span> <span>共<b>{{total}}</b>条结果</span>
    </div>
...
import {getSearchArticle, getSelectTagsArt} from "@/api/request";
...
const getTags = async ()=>{
  if(!obj.key) return
  const {data:res} = await getSelectTagsArt(toRaw(obj))
  console.log(res)
  if(res.code!=200) {
...
watch(()=>route.fullPath,()=>{
  obj.key = route.query.keyword as string
  getTags()
})
