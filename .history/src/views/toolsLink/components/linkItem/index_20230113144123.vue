<template>
  ...
  <div class="loadings"  element-loading-background="#F2F3F5" 
    element-loading-text="拼命加载中..." 
    v-if="!secondCate.length" 
    v-loading="true">
  </div></template>
  <template v-if="isDarkTheme">
  <div class="loadings" element-loading-background="#0A0A0A"  
    element-loading-text="拼命加载中..." 
    v-if="!secondCate.length " 
    v-loading="true">
  </div>
    ...
    <div  v-if="secondCate.length " class="categroy" v-for="item in secondCate" :key="item.id">
    <div class="title">
      <span>{{ item.name }}</span>
    </div>
