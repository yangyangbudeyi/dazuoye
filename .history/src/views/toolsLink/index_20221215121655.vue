...
const getSecondCate = (index:number)=>{
  console.log(index,linkData.allData,linkData.allData[index].sub)
...
onMounted(async ()=>{
  ...
  console.log(res)
  const datas = res.filter((item:any)=>item.sub?.length)
  console.log(datas)
  linkData.firstCate  = res.filter((item:any)=>item.sub?.length)?.map((item:any)=>item.name)
