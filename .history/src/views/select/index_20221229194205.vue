...
const activeName = ref(route.query.commit)
const userId = ref( route.params.userId as string)
const paramsId = ref("")

onMounted(()=>{
  paramsId.value = route.params.userId as string
  console.log(route.query.commit)
  router.replace(`/select/${paramsId.value}?commit=${activeName}`)
  getUser()
})
...
const handleClick = (tab: TabsPaneContext, event: Event) => {
  console.log(tab.paneName,route.fullPath)
  if(route.fullPath){
