        ...
        <div class="right">
          ...
          <div class="bott">
            ...
            <div class="huifu" v-if="!submitBtn">
              ...
              <div class="circleCss huiFuH" @click="huifu1(item.Pid, item.name)">
        ...
        <div class="right">
          ...
          <div class="bott">
            ...
            <div class="huifu" v-if="!submitBtn">
              ...
              <div class="circleCss huiFuH" @click="huifu1(item.Pid, item1.name)">
                <el-icon><ChatDotRound/></el-icon>
              </div>
            </div>
          </div>
...
let artId = ref(route.params.artId as string);
watch(
  () => route.fullPath,
  () => {
    console.log("发生变化了artId",route.params.artId)
...
let submitCom = reactive<{
  ...
  Pcontent: "",
  artId:artId.value,
  // artId: route.params.artId as string,
  userId,
  parent: "",
...
const getBtnStatus = async () => {
  const isDisable =await useValidateIDisabled("评论")
  if(isDisable) return submitBtn.value = true
}

//获取文章的评论内容
const getCommentByArtId = async () => {
  let { data: res } = await getCommentCountByIdAll(artId.value)
  data.commentList = res.data
}

//点击回复评论
const clickShowPingLun = (Pid:number|string) => {
  indexPid.value = Pid
}
//收起二级回复
const shouqi = (Pid?:any) => {
  indexPid.value = -1
}

const huifu1 =async (Pid:number|string, name:any) => {
...
if(!state||!login) return
  ...
  console.log(toRaw(submitCom))
  // let obj:ICommentDataParams = { ...toRaw(submitCom), Pdate: getCurTime() }
...
//删除评论
const deleteHuiFu = async(id:any, parent?:any) => {
  let preData = toRaw(data.commentList)
  let newData:any = preData?.filter((item:any) => item.Pid === id)
