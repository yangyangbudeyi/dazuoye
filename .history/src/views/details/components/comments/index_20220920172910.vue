      ...
      >
        ...
        <div class="right">
          ...
          <div class="bott">
            ...
            <div class="huifu" v-if="!submitBtn">
              <a
                v-if="item1.userId == $store.state.user.userInfo.userId"
...
// import isUserState from "@/hooks/useUserState"
import {emitter} from "@/emitter"
// let reqs = isUserState()
...
const getBtnStatus = async () => {
  const { data: res } = await selectSettingOne("评论")
...
const huifu1 = (Pid, name) => {
  if (!submitCom.userId || !localStorage.getItem("TOKEN")) {
    emitter.emit("loginSuccess", "123")
    ElMessage.warning("请先登录！")
    router.push("/login")
    return
  }
  // if (reqs.userStates.value == "0")
  //   return ElMessage.warning("您的账号状态异常！")
  dialogVisible.value = true
  if (Pid == 1 && name == 2) {
    submitCom.name = store.state.user.userInfo.name
...
const submitComments = async () => {
  ...
  let obj = { ...toRaw(submitCom), Pdate: getTime() }
