...
const getArticleByPreview = async (params?:string) => {
  ...
  setTimeout(() => {
    hotArticleByData.value = res.data?.filter((item:{artId:string})=>item.artId!=artId.value);;
...
const getRecommendArt = async () => {
  ...
  recommendArt.value = res.data?.filter((item:{artId:string})=>item.artId!=artId.value);
};
