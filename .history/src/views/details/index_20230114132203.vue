  ...
  <div class="details">
    <div class="left" ref="scrollTopMove">
      ...
      <HotArticle
        ...
        v-loading="hotArticleLoading"
        :data="hotArticleByData"
      ></HotArticle>
      <Tags v-loading="tagsLoading" :data="articleTagsData" :sticky="false"></Tags>
      <PageViewInfo
    ...
    <div class="right">
      <Loading :height="110"  style="margin-bottom:10px"  class="searchLoading" :key="Date.now()+1" :top="-205" v-if="!detailData.artContent"/>
...
const getRecommendArt = async () => {
  detailsLoading.value = true;
  const { data: res } = await getRandomArt(10)
  setTimeout(() => {
      detailsLoading.value = false;
      if (res.code != 200) {
    recommendArt.value = [];
