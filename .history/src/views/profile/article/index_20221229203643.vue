<template>
  ...
  <div v-show="!loading" class="Article">
    <div class="top">
      <el-tag size="large" plain
        >{{ isSearch ? "已为您查询到" : "您已经发表了" }}<b>{{ total }}</b
        >篇文章</el-tag
      >
      <div class="search">
        <el-input
      ...
      <div class="artItem" v-for="item in artData" :key="item.artId">
        ...
        <div class="button">
          <div
            ...
            @click="changeShow(item.artId ?? '', item.hidden ?? '')"
          >
            <el-icon v-if="item.hidden != '1'"><View /></el-icon>
            <el-icon v-else><Hide /></el-icon>
          </div>
          <div class="edit circleCss" @click="navigateEditor(item.artId ?? '')">
