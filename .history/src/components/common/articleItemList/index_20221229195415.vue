  ...
  <div class="item" @click="navigateDetails(item.artId??'')" v-for="item in list" :key="item.artId" >
    ...
    <div class="item_left">
      <div class="authOrTime">
        <div class="name"  @click.stop="navigateSelect(item.publishUserId ?? '',item.userId??'')">{{item.name}}</div>
...
const navigateSelect = (publishUserId:string,userId:string)=>{
  console.log(`publishUserId=${publishUserId},userId=${userId}`)
  ...
  }else if(route.fullPath.includes("?commit=article")||route.fullPath.includes("?commit=collect")){
    console.log(123)
    if(userIdStore==userId){
      router.push('/profile/userInfo')
    }else{
      router.push(`/select/${userId}?commit=info`)
